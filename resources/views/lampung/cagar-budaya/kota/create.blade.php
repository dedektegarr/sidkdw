...
@section('page', 'Tambah Cagar Budaya Kab/Kota')
{{-- @section('breadcrumbs', Breadcrumbs::render('bengkulu.cb-kabupaten-kota.create')) --}}
@section('content')
    <form method="POST" action="{{ route('cb.store') }}" class="max-w-lg mx-auto">
        ...
        <div class="mb-5">
            ...
            <input type="text" id="provinsi" value="Lampung" name="provinsi" readonly value="{{ old('provinsi') }}"
                class="shadow-sm bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 dark:shadow-sm-light"
                required />
            @error('provinsi')
                <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span class="font-medium">{{ $message }}</p>
