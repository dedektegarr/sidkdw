...
@section('page', 'OPK Bengkulu')
@section('breadcrumbs', Breadcrumbs::render('bengkulu.odcb'))
@section('content')
    <a href="{{ route('bengkulu.opk.create') }}"
    ...
    <table id="default-table">
        ...
        <tbody>
            @foreach ($data_opk as $opk)
                <tr>
                    <td class="font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $opk->nama_karya }}</td>
                    <td>{{ $opk->tahun }}</td>
                    <td>{{ $opk->sebaran }}</td>
                    <td>{{ $opk->domain->nama }}</td>
                    <td>{{ $opk->keterangan }}</td>
                    <td>
                        <a href="{{ route('bengkulu.opk.edit', $opk->id) }}"
                        ...
                        <button type="button" data-modal-target="delete-modal-{{ $opk->id }}"
                            data-modal-toggle="delete-modal-{{ $opk->id }}"
                        ...
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            ...
                            <div class="p-4 md:p-5 text-center">
                                ...
                                <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Apa kamu yakin ingin
                                    menghapus
                                    data ini?</h3>
                                <form action="{{ route('wbtb.destroy', $opk->id) }}" method="POST">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit"
                                ...
                                <button data-modal-hide="delete-modal-{{ $opk->id }}" type="button"
