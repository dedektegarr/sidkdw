    ...
    <table id="default-table">
        ...
        <tbody>
            @foreach ($data_domain as $domain)
                <tr>
                    <td class="font-medium text-gray-900 whitespace-nowrap dark:text-white">{{ $domain->nama }}</td>
                    <td>{{ $domain->keterangan ?? '-' }}</td>
                    <td>
                        <button type="button" data-modal-target="edit-modal-{{ $domain->id }}"
                            data-modal-toggle="edit-modal-{{ $domain->id }}"
                        ...
                        <button type="button" data-modal-target="delete-modal-{{ $domain->id }}"
                            data-modal-toggle="delete-modal-{{ $domain->id }}"
                ...
                {{-- EDIT MODAL --}}
                <div id="edit-modal-{{ $domain->id }}" tabindex="-1" aria-hidden="true"
                                    ...
                                    <div class="grid gap-4 mb-4 grid-cols-2">
                                        <div class="col-span-2">
                                            ...
                                            <input type="text" name="nama" id="nama"
                                                ...
                                                placeholder="Tulis nama domain" required=""
                                                value="{{ $domain->nama ?? old('nama') }}">
                                            @error('nama')
                                        ...
                                        <div class="col-span-2">
                                            ...
                                            <textarea id="keterangan" rows="4" name="keterangan"
                                                ...
                                                placeholder="Tulis keterangan">{{ $domain->keterangan ?? old('keterangan') }}</textarea>
                                        </div>
                                    </div>
                                    <button type="submit"
                ...
                {{-- DIALOG MODAL --}}
                <div id="delete-modal-{{ $domain->id }}" tabindex="-1"
                    class="hidden overflow-y-auto overflow-x-hidden fixed top-0 right-0 left-0 z-50 justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full">
                    <div class="relative p-4 w-full max-w-md max-h-full">
                        <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                            <button type="button"
                                class="absolute top-3 end-2.5 text-gray-400 bg-transparent hover:bg-gray-200 hover:text-gray-900 rounded-lg text-sm w-8 h-8 ms-auto inline-flex justify-center items-center dark:hover:bg-gray-600 dark:hover:text-white"
                                data-modal-hide="delete-modal-{{ $domain->id }}">
                                <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg"
                                    fill="none" viewBox="0 0 14 14">
                                    <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round"
                                        stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6" />
                            ...
                            <div class="p-4 md:p-5 text-center">
                                ...
                                <button data-modal-hide="delete-modal-{{ $domain->id }}" type="button"
    ...
    <div id="add-modal" tabindex="-1" aria-hidden="true"
        ...
        <div class="relative p-4 w-full max-w-md max-h-full">
            ...
            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                ...
                <!-- Modal body -->
                <form class="p-4 md:p-5" method="POST" action="{{ route('domain.store') }}">
                    @csrf
                    <div class="grid gap-4 mb-4 grid-cols-2">
                        <div class="col-span-2">
                            <label for="nama"
                            ...
                            <input type="text" name="nama" id="nama"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500"
                                placeholder="Tulis nama Domain" required="" value="{{ old('nama') }}">
                            @error('nama')
                                <p class="mt-2 text-sm text-red-600 dark:text-red-500"><span
                                        class="font-medium">{{ $message }}</p>
                            @enderror
